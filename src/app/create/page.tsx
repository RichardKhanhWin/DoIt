			...
			<form onSubmit={onSubmit} className="flex items-center justify-center w-1/2 m-auto h-screen">
				<div className="flex grow items-center flex-col gap-y-[8px]">
					<div className="flex min-w-full justify-between gap-x-[4px]">
						<input name="title" defaultValue={""} className="text-box h-8 grow" placeholder="Title (required)" onChange={e => dispatch({ type: "onTitleChange", title: e.target.value })} required maxLength={50}/>
						<Checkbox name="done" defaultChecked={false} className="checkbox h-8 w-8 border-2" />
					</div>
					<div className="flex min-w-full">
						<textarea name="description" rows={10} className="text-box grow resize-none" placeholder="Description" />
					</div>
					<div>
						<button type="submit" className={`button p-2 rounded-md ${state.isLoading && "opacity-50"}`} disabled={state.isLoading}>
