			...
			<form action={editItem} className="flex grow items-center">
				<div className="flex items-center flex-col min-w-full gap-y-[8px]">
					<div className="flex min-w-full justify-between gap-x-[4px]">
						<input name="title" defaultValue={item?.title} className="dark:bg-slate-700 dark:text-white p-1 grow" placeholder="Title (required)" required maxLength={50} />
						<Checkbox name="done" defaultChecked={item?.done} className="h-8 w-8 bg-slate-200 border-black border-2 text-black" />
					</div>
					<div className="flex min-w-full">
						<textarea name="description" rows={10} defaultValue={item?.description || ""} className="dark:bg-slate-700 dark:text-white p-1 grow resize-none" placeholder="Description" />
					</div>
					<div>
